      ...
      <div v-else class="comment-items">
        <div v-for="comment in comments" :key="comment.id" class="comment-item">
          ...
          <div class="comment-actions" v-if="userStore.islogin">
            <el-button type="primary" link @click="showReplyInput(comment.id)">
              回复
            </el-button>
          </div>
          ...
          <div class="reply-input" v-if="activeReplyId === comment.id">
            ...
            <div class="reply-actions">
              <el-button type="danger" size="small" @click="cancelReply">取消</el-button>
          ...
          <div class="reply-list" v-if="comment.replies && comment.replies.length > 0">
            ...
            <div class="reply-item">
              <div class="comment-header">
                <el-avatar :size="24" :src="comment.replies[0].avatar" />
                <div class="comment-info">
                  <span class="username">
                    {{ comment.replies[0].username }}
                    <n-tag size="small" v-if="comment.replies[0].userid === props.authorId" type="success">作者</n-tag>
                  </span>
                  <span class="time">{{ formatDate(comment.replies[0].createTime) }}</span>
                </div>
              </div>
              <div class="reply-content">
                <template v-if="comment.replies[0].replyToUsername">
                  回复 <span class="reply-to">@{{ comment.replies[0].replyToUsername }}</span>：
                </template>
                {{ comment.replies[0].content }}
              </div>
              <div class="comment-actions" v-if="userStore.islogin">
                <el-button type="primary" link @click="showReplyInput(comment.id, comment.replies[0].userid, comment.replies[0].username)">
                  回复
                </el-button>
              </div>
            </div>
            ...
            <!-- 折叠的回复 -->
            <div v-if="comment.replies.length > 1">
              <div v-if="expandedComments[comment.id]">
                <div v-for="reply in comment.replies.slice(1)" :key="reply.id" class="reply-item">
                  <div class="comment-header">
                  ...
                  <div class="comment-actions" v-if="userStore.islogin">
                    <el-button type="primary" link @click="showReplyInput(comment.id, reply.userid, reply.username)">
                      回复
                    </el-button>
                  </div>
                </div>
              ...
              <div class="expand-button">
                <el-button type="primary" link @click="toggleReplies(comment.id)">
                  {{ expandedComments[comment.id] ? '收起回复' : `展开${comment.replies.length - 1}条回复` }}
                </el-button>
              </div>
            </div>
          </div>
...
import { getComments, addComment, addReply, addMsg } from '@/api/api';
